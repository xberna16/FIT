...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  return (int *)(head->buffer + head->stride * gl_VertexID + head->offset);
}
...
void gpu_runVertexPuller(
  ...
  {
    if (&(puller->heads[i]) == NULL || &puller->heads[i].enabled == NULL)
    {
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  float pointX = pixelCenter->data[0];
  float pointY = pixelCenter->data[1];
  ...
  coords->data[0] = (((y2 - y3)*(pointX - x3))+((x3 - x2)*(pointY - y3))) / (((y2 - y3)*(x1 - x3)) + (x3 - x2)*(y1 - y3));
  coords->data[1] = (((y3 - y1)*(pointX - x3))+((x1 - x3)*(pointY - y3))) / (((y2 - y3)*(x1 - x3)) + (x3 - x2)*(y1 - y3));
